...
namespace WinFormsApp_final
{
    ...
    {
        // 加载推理相关方法
        [DllImport("model_infer.dll", EntryPoint = "InitModel")] // 模型初始化方法
        // paddlex_model_type: cpp -- > char *
        // c# -- > byte[] size:10
        // 存储paddlex--模型yml中，对应的model_type -- det,seg
        public static extern void InitModel(string model_type, string model_filename, string params_filename, string cfg_file, bool use_gpu, ref byte paddlex_model_type);

        [DllImport("model_infer.dll", EntryPoint = "Det_ModelPredict")]  // 模型推理方法
        public static extern void Det_ModelPredict(byte[] img, int W, int H, int C, IntPtr output, int[] BoxesNum, ref byte label);

        [DllImport("model_infer.dll", EntryPoint = "Seg_ModelPredict")]  // 模型推理方法
        public static extern void Seg_ModelPredict(byte[] img, int W, int H, int C, ref byte output);

        [DllImport("model_infer.dll", EntryPoint = "Cls_ModelPredict")]  // 模型推理方法
        ...
        [DllImport("model_infer.dll", EntryPoint = "DestructModel")]  // 模型销毁方法
        ...
        // 模型文件类型
        //string[] file_type = { "model.pdmodel", "model.pdiparams", "model.yml" };
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (comboBox2.SelectedItem.ToString() == "det")  // 加载检测模型  -- 推理已实现
            {
                ...
                paddlex_doing = false;  // 进入非paddlex模式
            }
            else if (comboBox2.SelectedItem.ToString() == "seg") // 加载分割模型 -- 推理未实现
            {
                model_type = comboBox2.SelectedItem.ToString();
                paddlex_doing = false;  // 进入非paddlex模式
            }
            else if (comboBox2.SelectedItem.ToString() == "clas") // 加载识别模型 -- 推理未实现
            {
                model_type = comboBox2.SelectedItem.ToString();
                paddlex_doing = false;  // 进入非paddlex模式
            }
            else if (comboBox2.SelectedItem.ToString() == "paddlex") // 加载识别模型 -- 推理未实现
            {
                model_type = comboBox2.SelectedItem.ToString();
                // 重复选中paddlex，不修改状态
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (dir_load_flag == 1) // 寻找模型文件
            {
                ...
                if (model_lst.Count != 1 || params_lst.Count != 1 || cfg_lst.Count != 1)
                {
                    MessageBox.Show("模型文件加载失败!\n请注意模型文件夹下应该只包含以下文件各一个:\n*.pdmodel, *.pdiparams, *.yml/*.yaml", "提示");
                    return;
                }
                ...
                if (has_model_init == 1) // 已经初始化，再次初始化前要完成上一个模型的销毁
                {
                    ...
                    try
                    {
                        ...
                        InitModel(model_type, model_filename, params_filename, cfg_file, use_gpu, ref paddlex_model_type[0]);
                        ...
                        if (model_type == "paddlex") // 如果当前初始模型类型为paddlex，则初始化完成后，转为paddlex模型的实际类型
                        {
                            ...
                            // "abcd" --> string
                            // "abcd\0"
                            // paddlex_model_type == 10, det ==> "det\0\0\0\0..."
                            // "det" - >string --> \0
                    ...
                    catch (Exception ex)
                    {
                        ...
                        MessageBox.Show("1.请确定文件中包含有效的模型文件(*.pdmodel, *.pdiparams, *.yml)!\n2.请检查模型文件与模型类型是否一致!", "模型初始化失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
                {
                ...
                if (raise_ex_flag == 0) // 未发生异常时，才进行正常的运行提示
                {
                    ...
                    MessageBox.Show($"模型文件已加载!(模型类型为: {model_type.Split('\0')[0]})", "提示");
        ...
        private void det_infer_one_img()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < boxesInfo[0]; i++) // 未绘制图像
                {
                    ...
                    {
                        /*
                        int[] color_ = { (int)(color_map[(labelindex%79)*3] * 255),
                                             (int)(color_map[(labelindex % 79) * 3 + 1] * 255),
                                             (int)(color_map[(labelindex % 79) * 3 + 2] * 255) };
                        */
                        ...
                        Cv2.PutText(mat, $"{predict_Label_List[i]}-{labelindex}-: {score:f2}", new OpenCvSharp.Point((int)left, (int)top-5), HersheyFonts.HersheySimplex, 1, new OpenCvSharp.Scalar(color_[0], color_[1], color_[2]), 2, LineTypes.Link4);
                    }
                }
            ...
            catch (Exception e)
            {
                ...
                MessageBox.Show("请检查模型文件与模型类型是否一致!", "模型运行失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void seg_infer_one_img()
        {
            ...
            {
                TimeSpan infer_start_time = new TimeSpan(DateTime.Now.Ticks);
                // public static extern void Seg_ModelPredict(byte[] img, int W, int H, int C, byte[] output);
                //第四个参数为输入图像的通道数
        ...
        private void seg_infer_many_img()
        {
            ...
            try
            {
                foreach (string img_file in imgfiles)
                {
                    ...
                    Bitmap origin_bmp = new Bitmap(img_file);
                    Bitmap input_bmp = null;
                    ...
                    Cv2.AddWeighted(output_mat, 0.8, input_mat, 0.2, 1, output_mat);  // 执行叠加
                    input_bmp = OpenCvSharp.Extensions.BitmapConverter.ToBitmap(output_mat);
