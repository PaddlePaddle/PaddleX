...
from ._config_pp_ocrv6_medium_det import PPOCRV6MediumDetConfig


class PPOCRV6MediumDetConvBatchnormLayer(nn.Layer):
    def __init__(
        self,
        in_channels,
        out_channels,
...
class PPOCRV6MediumDetIntraclassBlock(nn.Layer):
...
class PPOCRV6MediumDetNeck(nn.Layer):
    def __init__(self, config):
        ...
        self.num_backbone_stages = len(backbone_stage_output_channels)
        ...
        for stage_index in range(self.num_backbone_stages):
            self.input_channel_adjustment_convolution.append(
                nn.Conv2D(
                    in_channels=backbone_stage_output_channels[stage_index],
    ...
    def forward(self, backbone_stage_feature_maps):
        ...
        for i, feature_map in enumerate(backbone_stage_feature_maps):
            channel_adjusted.append(
                self.input_channel_adjustment_convolution[i](feature_map)
            )

        top_down = [None] * self.num_backbone_stages
        top_down[-1] = channel_adjusted[-1]
        ...
        for i in range(self.num_backbone_stages):
            ...
            projected.append(self.input_feature_projection_convolution[i](hidden_states))

        bottom_up = [None] * self.num_backbone_stages
        bottom_up[0] = projected[0]
        for i in range(1, self.num_backbone_stages):
            bottom_up[i] = projected[i] + self.path_aggregation_head_convolution[
                i - 1
            ](bottom_up[i - 1])
...
class PPOCRV6MediumDetHead(nn.Layer):
    def __init__(self, config):
        super().__init__()
        in_channels = config.neck_out_channels
...
class PPOCRV6MediumDetModel(nn.Layer):
    def __init__(self, config):
        super().__init__()
...
class PPOCRV6MediumDet(BatchNormHFStateDictMixin, PretrainedModel):
