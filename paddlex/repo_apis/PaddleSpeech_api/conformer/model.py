...
class ChunkConformerModel(BaseModel):
    """Chunk-based Conformer model for streaming speech recognition"""
    ...
    def __init__(self, config):
        ...
        self.chunk_size = config.get("chunk_size", 32)
        self.left_context = config.get("left_context", 4)
    ...
    def train(
        ...
        **kwargs,
        ...
        Args:
            ...
            epochs_iters (int, optional): Number of epochs/iterations to train.
            pretrained (str, optional): Path to pretrained weights.
            ips (str, optional): Ips for distributed training.
            device (str): Device type (gpu/cpu).
            resume_path (str, optional): Path to checkpoint to resume from.
            dy2st (bool): Whether to use dynamic-to-static conversion.
            amp (str): Auto mixed precision mode (OFF/O1/O2).
            num_workers (int, optional): Number of workers for data loading.
            use_vdl (bool): Whether to use VisualDL logging.
    ...
    def _assert_empty_kwargs(self, kwargs):
        if kwargs:
            raise ValueError(f"Unsupported arguments: {', '.join(kwargs.keys())}")

    def forward(self, audio_features, cache=None):
        """
        Args:
            audio_features (Tensor): [B, T, D]
            cache (Tensor|None): Previous chunk state for streaming
        """
        if cache is not None:
            # Apply overlap-add for chunk processing
            audio_features = self._apply_context(audio_features, cache)

        return audio_features  # Removed encoder layers processing

    def _apply_context(self, current_chunk, cache):
        """Apply left context from previous chunks"""
        B, T, D = current_chunk.shape
        if cache is None:
            context = paddle.zeros([B, self.left_context, D])
        else:
            context = cache[:, -self.left_context :, :]
        padded = paddle.concat([context, current_chunk], axis=1)
        return padded[:, : T + self.left_context]
