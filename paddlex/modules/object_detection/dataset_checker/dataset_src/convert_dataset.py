...
def convert(dataset_type, input_dir, output_dir):
    ...
    convert_voc_dataset(input_dir, anno_map, output_dir) if dataset_type in (
        "VOC", "VOCWithUnlabeled") else convert_labelme_dataset(
            input_dir, anno_map, output_dir)
    return output_dir
...
def convert_labelme_dataset(root_dir, anno_map, output_dir):
    ...
    if os.path.exists(output_dir):
        print(f"The output_dir ({output_dir}) exists, would be overrided.")
...
def convert_voc_dataset(root_dir, anno_map, output_dir):
...
def voc_get_label_anno(root_dir, anno_path):
    ...
    ann_paths = []
    print(f"Start loading xml annotation files from {anno_path} ...")
    for aid in ann_ids:
        aid = aid.strip().split(' ')[-1]
        if not aid.endswith('.xml'):
            print(f"An illegal xml path(\"{aid}\") found! Has been ignored.")
            continue
...
def voc_xmls_to_cocojson(root_dir, annotation_paths, label_indexer, img_indexer,
    ...
    with custom_open(output_file, 'w') as f:
        output_json = json.dumps(output_json_dict)
        f.write(output_json)
    print(f"The converted annotations has been save to {output_file}.")
