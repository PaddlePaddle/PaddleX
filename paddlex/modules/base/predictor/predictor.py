...
Env: {self.kernel_option}"
        ...
        self.pre_tfs, self.post_tfs = self.build_transforms(
            pre_transforms, post_transforms
        )

        self._predictor = _PaddleInferencePredictor(
            model_dir=model_dir, model_prefix=self.MODEL_FILE_TAG, option=kernel_option
        )
    ...
    def predict(self, input, batch_size=1):
        """predict"""
        if not isinstance(input, dict) and not (
            isinstance(input, list) and all(isinstance(ele, dict) for ele in input)
        ):
        ...
        for mini_batch in Batcher(input, batch_size=batch_size):
            mini_batch = self._preprocess(mini_batch, pre_transforms=self.pre_tfs)
            ...
            mini_batch = self._postprocess(mini_batch, post_transforms=self.post_tfs)
...
def create_model(
    ...
    **kwargs,
    ...
    kernel_option = PaddleInferenceOption() if kernel_option is None else kernel_option
    if model_dir is None:
        if model_name in official_models:
            model_dir = official_models[model_name]
