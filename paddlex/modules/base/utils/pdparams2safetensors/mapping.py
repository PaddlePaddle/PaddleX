...
RTDETR_MAPPING = [
    # === Backbone: HGNetV2 ===
    # Stem
    (r"^backbone\.stem\.(\w+)\.conv\.(\w+)$", r"model.backbone.model.embedder.\1.convolution.\2"),
    (r"^backbone\.stem\.(\w+)\.bn\.(\w+)$", r"model.backbone.model.embedder.\1.normalization.\2"),
    # Stages: aggregation
    (r"^backbone\.stages\.(\d+)\.blocks\.(\d+)\.aggregation_squeeze_conv\.conv\.(\w+)$",
    ...
    (r"^neck\.encoder\.(\d+)\.layers\.(\d+)\.linear1\.", r"model.encoder.encoder.\1.layers.\2.fc1."),
    (r"^neck\.encoder\.(\d+)\.layers\.(\d+)\.linear2\.", r"model.encoder.encoder.\1.layers.\2.fc2."),
    ...
    # FPN blocks (.bn → .norm)
    (r"^neck\.fpn_blocks\.(\d+)\.((?:bottlenecks\.\d+\.)?conv\d)\.bn\.", r"model.encoder.fpn_blocks.\1.\2.norm."),
    (r"^neck\.fpn_blocks\.", r"model.encoder.fpn_blocks."),
    # PAN blocks (.bn → .norm)
    (r"^neck\.pan_blocks\.(\d+)\.((?:bottlenecks\.\d+\.)?conv\d)\.bn\.", r"model.encoder.pan_blocks.\1.\2.norm."),
    ...
    (r"^neck\.mask_feat_head\.output_conv\.conv\.", r"model.encoder.mask_feature_head.output_conv.convolution."),
    (r"^neck\.mask_feat_head\.output_conv\.bn\.", r"model.encoder.mask_feature_head.output_conv.normalization."),
    # Encoder mask lateral/output (DocLayoutV3 only)
    (r"^neck\.enc_mask_lateral\.conv\.", r"model.encoder.encoder_mask_lateral.convolution."),
    (r"^neck\.enc_mask_lateral\.bn\.", r"model.encoder.encoder_mask_lateral.normalization."),
    (r"^neck\.enc_mask_output\.0\.conv\.", r"model.encoder.encoder_mask_output.base_conv.convolution."),
    (r"^neck\.enc_mask_output\.0\.bn\.", r"model.encoder.encoder_mask_output.base_conv.normalization."),
    (r"^neck\.enc_mask_output\.1\.", r"model.encoder.encoder_mask_output.conv."),

    # === Transformer → model.decoder + heads ===
    # Decoder input_proj
    (r"^transformer\.input_proj\.(\d+)\.conv\.(\w+)$", r"model.decoder_input_proj.\1.0.\2"),
    (r"^transformer\.input_proj\.(\d+)\.norm\.(\w+)$", r"model.decoder_input_proj.\1.1.\2"),
...
UVDOC_MAPPING = [
    ...
    (r"^out_point_positions2D\.2\._weight$", r"head.out_point_positions2D.conv_down.activation.weight"),
...
# ---------------------------------------------------------------------------
# Keys to drop during rec model conversion
# ---------------------------------------------------------------------------
