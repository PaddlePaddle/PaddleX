...
![](https://rte.weiyun.baidu.com/wiki/attach/image/api/imageDownloadAddress?attachId=29b6ec6c456a4125bdebd81575aab00e&docGuid=qaEbM1Zud3LJA9 "")
...
![](https://rte.weiyun.baidu.com/wiki/attach/image/api/imageDownloadAddress?attachId=e731cd1fdc874cc99b3e1e3aafd3462e&docGuid=cp9ThT9KR5WF1X "")
...
```
执行后，将提示选择时序异常检测产线配置文件保存路径，默认保存至*当前目录*，也可 *自定义路径*。
...
* 实例化 `create_pipeline` 实例化时序异常检测产线对象：具体参数说明如下：
...
* 调用时序异常检测产线对象的 `predict` 方法进行推理预测：`predict` 方法参数为`x`，用于输入待预测数据，支持多种输入方式，具体示例如下：

|参数类型|参数说明|
|-|-|
|Python Var|支持直接传入Python变量，如numpy.ndarray表示的图像数据；|
|str|支持传入待预测数据文件路径，如图像文件的本地路径：/root/data/img.jpg；|
|str|支持传入待预测数据文件url，如图像文件的网络url：https://paddle-model-ecology.bj.bcebos.com/paddlex/ts/demo_ts/ts_ad.cs；|
...
若您需要将通用时序异常检测产线直接应用在您的 Python 项目中，可以参考 2.2.2 Python脚本方式 中的示例代码。
...
### 4.1 模型微调
由于通用时序异常检测产线包含时序异常检测模块，如果模型产线的效果不及预期，那么您需要参考[时序预测模块开发教程](../../../module_usage/tutorials/time_series_modules/time_series_anomaly_detection.md)中的**二次开发**章节，使用您的私有数据集对时序异常检测模型进行微调。
