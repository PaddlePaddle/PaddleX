...
```
6. 3.3. Windows 安装 ADB
7. win 上安装需要去谷歌的安卓平台下载 ADB 软件包进行安装：[链接](https://developer.android.com/studio)
8. 打开终端，手机连接电脑，在终端中输入
```
 adb devices
```
9. 如果有 device 输出，则表示安装成功。
```
...
下面是 object_detection 解压操作示例，其他产线可参考后面的表格。
...
4. 将工作目录切换到 `PaddleX-Lite-Deploy/{Pipeline_Name}/android/shell/cxx/{Demo_Name}`，运行 `run.sh` 脚本，完成在端侧的预测。
5. **注意：**
...
![](https://rte.weiyun.baidu.com/wiki/attach/image/api/imageDownloadAddress?attachId=f0481272f0574ddaa387c529dc2656af&docGuid=WgkMGzkjzQlsxg "")
