...
1. 若您需要使用微调后的模型权重，只需对产线配置`paddlex/configs/pipelines/ts_classification.yaml`文件做修改，将微调后模型权重的本地路径填写至产线配置文件中的 `model_dir` 即可, 若您需要将通用时序分类产线直接应用在您的 Python 项目中，可以参考 如下示例：
...
pipeline = create_pipeline(pipeline="paddlex/configs/pipelines/ts_classification.yaml")
