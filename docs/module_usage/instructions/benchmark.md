...
* `PADDLE_PDX_INFER_BENCHMARK_USE_CACHE_FOR_READ`：设置为 `True` 时则对读取输入数据操作应用缓存机制，避免重复I/O开销，并且并且数据读取及缓存消耗的时间不记录到核心耗时中。默认为 `False`；
* `PADDLE_PDX_INFER_BENCHMARK_USE_NEW_INFERENCE_API`：设置为 `True` 时则使用新的推理API，可以看更细致的分阶段结果。默认为 `False`；
