...
<b>通用版面解析产线中包含必选的版面区域分析模块</b>以及可选的文档图像预处理子产线、通用OCR子产线、表格识别子产线、印章识别子产线和公式识别子产线。
...
```
<b>注：</b> 配置文件中的参数为产线初始化参数，如果希望更改通用版面解析产线初始化参数，可以直接修改配置文件中的参数，并加载配置文件进行预测。同时，CLI 预测也支持传入配置文件，`--pipeline` 指定配置文件的路径即可。此外，`save_to_img` 能够保存可视化结果（包括版面区域检测结果图片、OCR 结果图片、表格结构识别结果图片、公式识别结果图片和印章识别结果图片）， `save_to_html` 能够将表格直接保存为html文件（包括文本和表格格式），`save_to_xlsx` 能够将表格保存为Excel格式文件（包括文本和格式）。
