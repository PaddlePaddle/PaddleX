...
### 4.1 模型微调
由于通用时序异常检测产线包含时序异常检测模块，如果模型产线的效果不及预期，那么您需要参考[时序预测模块开发教程](https://paddlepaddle.github.io/PaddleX/latest/module_usage/tutorials/time_series_modules/time_series_anomaly_detection.html)中的[二次开发](https://paddlepaddle.github.io/PaddleX/latest/module_usage/tutorials/time_series_modules/time_series_anomaly_detection.html#四二次开发)章节，使用您的私有数据集对时序异常检测模型进行微调。
