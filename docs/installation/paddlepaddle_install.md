...
- 对于CUDA 11.8 + cuDNN8.9，兼容的 TensorRT 版本为8.x（x>=6）。PaddleX 已在 TensorRT 8.6.1.6 上完成了 Paddle-TensorRT 的兼容性测试，因此**强烈建议安装 TensorRT 8.6.1.6**。
- 对于CUDA 12.6 + cuDNN9.5，兼容的 TensorRT 版本为10.x（x>=5），建议安装 TensorRT 10.5.0.18。
