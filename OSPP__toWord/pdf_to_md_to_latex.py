...
def md_to_latex(md_text: str, output_name: str, output_path):
    ...
    for idx, page in enumerate(content_pages):
        ...
        if headers and idx < len(headers):
            latex_lines.append(
                f"\\fancyhead[C]{{{escape_latex_outside_formula(headers[idx])}}}"
            )
        if footers and idx < len(footers):
            latex_lines.append(
                f"\\fancyfoot[C]{{{escape_latex_outside_formula(footers[idx])}}}"
            )
        ...
        if page_numbers and idx < len(page_numbers) and page_numbers[idx]:
            latex_lines.append(
                f"\\begin{{center}}{escape_latex_outside_formula(page_numbers[idx])}\\end{{center}}\n"
            )
