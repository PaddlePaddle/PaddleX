...
bool DetPreProcess::PrepareInputs(
  ...
  std::cerr << "Unsupported model type of '"
            << model_arch_ << "' " << std::endl;
  return false;
}
...
bool DetPreProcess::PrepareInputsForRCNN(
  ...
  for (auto i = 0; i < batch; ++i) {
    ...
    float resize_w = static_cast<float>(
                    shape_infos[shapes_num - 2].shapes[0][0]);
    float scale = resize_w / origin_w;
    float im_info_data[] = {static_cast<float>(h),
                        static_cast<float>(w), scale};
    float im_shape_data[] = {origin_h, origin_w, 1.0};
    memcpy(im.data.data() + i * sample_shape * sizeof(float),
                imgs[i].data, sample_shape * sizeof(float));
    memcpy(im_info.data.data() + i * 3 * sizeof(float),
                im_info_data, 3 * sizeof(float));
    memcpy(im_shape.data.data() + i * 3 * sizeof(float),
                im_shape_data, 3 * sizeof(float));
  }
